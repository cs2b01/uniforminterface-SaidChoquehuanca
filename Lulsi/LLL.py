...
def campo_minas1():
    opcion = input("¿Desea jugar?(Si = 1) o (Personalizable = 0) :")
    if opcion:
        while True:
            print("Seleccione la dificultad: \n")
            print("1. Nivel Facil (f).\n")
            print("2. Nivel Intermedio(i).\n")
            opcion = input(str("Elija una opcion:"))
            if opcion == ("f"):
...
def jugada(filas, columnas):
    while True:
        fila = input(int("Ingrese la fila: "))
        columna = input(int("Ingrese la columna: "))
        if ((1 <= fila) and (fila <= filas)) and ((1 <= columna) and (columna <= columnas)):
            break
